...
\hline
Primary Scope & \shortstack[l]{Virtualized wired IEEE 802.1X NAC proof-of-concept with policy-based guest access\\
enforcement, incident simulation, automation, and packet-level validation} \\
\hline
Primary Technologies & \shortstack[l]{IEEE 802.1X via \texttt{hostapd} (wired mode) and \texttt{wpa\_supplicant}; pfSense;\\
FreeRADIUS; GLPI on Apache and MariaDB; Splunk Universal Forwarder; Python; Ansible; \texttt{tcpdump}; Wireshark} \\
\hline
VM Platform & VMware Workstation Pro: Ubuntu Server 24.04 LTS \\
\hline
Primary Lab Purpose & \shortstack[l]{Demonstrate a realistic small-scale NAC architecture with authenticated access\\
control, constrained guest access, observability, ITSM workflow integration, automation, and protocol validation} \\
\hline
\end{tabularx}
\end{center}
