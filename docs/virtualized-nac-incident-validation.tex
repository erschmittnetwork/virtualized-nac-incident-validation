...
\hline
Primary Scope & Virtualized wired 802.1X NAC proof-of-concept with policy-based guest access \\
        enforcement, incident simulation, automation, and packet-level validation \\
\hline
Primary Technologies & IEEE 802.1X via hostapd (wired mode) + wpa_supplicant, \\
        pfSense, FreeRADIUS, GLPI on Apache + MariaDB, Splunk Universal Forwarder, \\
        Python, Ansible, tcpdump, Wireshark \\
\hline
...
Primary Lab Purpose & Demonstrate a realistic small-scale NAC architecture with authenticated access \\
        control, constrained guest access, observability, ITSM workflow integration, automation, \\
