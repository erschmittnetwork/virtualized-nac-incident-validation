...
--
-- Purpose:
--   Rewrite appendix header numbering and internal reference text for HTML
--   generated by Pandoc from LaTeX input.
--
-- Behavior:
--   - Body references to appendix sections become:
--       Section A.1.2
--       Section B.2.4
--   - Top-level appendix headers become:
--       Appendix A. Raw Configurations and Automation Artifacts
--       Appendix B. Incident, Validation, and Evidence Artifacts
--   - Lower-level appendix headers become:
--       A.1 Host Network Configurations
--       A.2.4 FreeRADIUS Clients Configuration
--
-- Assumptions:
--   - Appendix labels follow patterns like:
--       sec:appendix-a-...
--       sec:appendix-b-...
--   - Appendix A and Appendix B are top-level section headers.
--   - Some appendix titles may already contain manual numbering, which
--     this filter strips before inserting normalized numbering.
